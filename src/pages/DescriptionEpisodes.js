...
import PalyerEpisodes from "../components/PalyerEpisodes";
  ...
  return data[0] ? (
    ...
    >
      ...
      <div className="flex flex-col justify-center items-center gap-4 mt-96">
        <Typography text={`Brak dalszych odcinków`} size={"text-4xl"} />
        <NavLink
          className="group flex justify-center items-center bg-slate-900 p-2 rounded gap-3 text-4xl order-2"
          to={location.slice(0, location.length - slugEpisodesLength)}
          style={{ color: "white" }}
        >
          <IconContext.Provider
            value={{ className: "text-4xl group-hover:text-yellow-500" }}
          >
            <TbBrandCodesandbox />
            Lista odcinków
          </IconContext.Provider>
