...
const Home = () => {
    ...
    useEffect(() => {
        if(dataAnime[0]){
        setData(dataAnime.filter((value) => value.season_year === new Date().getFullYear() && value.season === changeSeazon()))
        }
    ...
    return (
        <div className=' flex flex-col overflow-hidden justify-start items-center min-h-screen w-[80vw] mx-auto' style={{ maxWidth: 'calc(100vw - 81px)'}}>
            ...
            <h1 className="text-xl font-medium text-white">{`${changeSeazon("pl")} ${new Date().getFullYear()}`}</h1>
         <Swiper
           autoplay={{
            delay: 2500,
      ...
      >
            {data? data.map((item,key) =>  (  
                 ...
                 <SwiperSlide style={{marginTop: "0",}} className="bg-cover " key={key}>
                 <NavLink  to={`/anime/${item.slug}`}>
                    ...
                    <img src={item.cover} alt="" className="rounded-xl object-cover h-[300px] "/>
                    </div>
                    <div className="absolute bottom-0 left-0 w-4/5 text-white font-bold m-3 text-sm">{item.title}</div>
                    </NavLink>
                </SwiperSlide>)) :"aa"}
      </Swiper>
