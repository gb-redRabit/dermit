...
const Filmy = ({animeMovie}) => {
    ...
    const searchInput =(e)=>{
        ...
        setData(animeMovie.filter(word=> word.title.toLowerCase().includes(text)))
    }
    const displayGenres=()=>{
    ...
    function changeBackground(e) {
      ...
      const clickGenres =(e)=>{
        ...
        setData(animeMovie.filter(word=> [...word.genres].includes(e.target.innerText)))
    ...
    return (
        <div className='flex  flex-col justify-start items-start flex-wrap  min-h-screen w-[80vw] mx-auto' style={{ maxWidth: 'calc(100vw - 81px)'}}>
            ...
            <div className='mb-10 flex bg-slate-900 w-full rounded p-4' style={genresDisplay?{display: 'none'}:{display: "flex"}}>
